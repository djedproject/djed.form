...
ID_FIELD = 'djed.form:field'
ID_PREVIEW = 'djed.form:field-preview'


def add_field(cfg, name, cls):
    """ Field registration directive. Field should be inherited from
    :py:class:`djed.form.Field` class.

    .. code-block:: python

      class TextField(djed.form.Field):
          ...
    ...
    intr = Introspectable(ID_FIELD, discr, name, 'djed.form-field')
    intr['name'] = name
    intr['field'] = cls
...
class field(object):
    ...
    def __call__(self, wrapped):
        ...
        info = venusian.attach(wrapped, callback, category='djed.form')
        return wrapped
...
def add_fieldpreview(cfg, cls, callable):
    ...
    intr = Introspectable(
        ID_PREVIEW, discr, 'Field preview for "%s"'%cls, 'djed.form-preview')
    intr['field'] = cls
...
class fieldpreview(object):
    ...
    .. code-block:: python
      ...
      def textPreview(request):
          ...
          widget.update()
          return djed.templates.render(request, 'form:widget', widget)
