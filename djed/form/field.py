...
class _Field(object):
    ...
    ``validator``: Optional validator for this field.  It should be
      ...
      :py:class:`djed.form.interfaces.Validator` interface.

    ``default``: Default field value.

    ``missing``: Field value if value is not specified in bound value.

    ``error``:: Instance os ``djed.form.interfaces.Invalid`` class or None.
...
class FieldFactory(Field):
    ...
    with :py:func:`djed.form.field` decorator.
    ...
    def bind(self, request, prefix, value, params, context=None):
        try:
            cls = request.registry['djed.form:field'][self.__field__]
        except KeyError:
            cls = None
